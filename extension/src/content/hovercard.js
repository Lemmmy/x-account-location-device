...
function buildCardContent({ screenName, info, loading = false, errorText = "" }) {
  ...
  if (isTrustedTwimgUrl(avatarUrl)) {
    ...
    const fallback = createEl(
      "div",
      "x-posed-avatar-fallback",
      (screenName || "?").slice(0, 1).toUpperCase(),
    );
  ...
  if (meta.blueVerified) {
    tags.appendChild(
      createTag({ label: "Blue", tone: "blue", title: "X Premium / Blue verified" }),
    );
  }
  if (meta.verified) {
  ...
  if (meta.protected) {
    tags.appendChild(
      createTag({ label: "Protected", tone: "neutral", title: "Protected account" }),
    );
  }
  ...
  if (aff?.name) {
    tags.appendChild(
      createTag({ label: aff.name, tone: "purple", title: "Affiliation / business label" }),
    );
  }
  ...
  if (info?.location) {
    body.appendChild(
      createRow({ icon: "📍", label: "Location", value: safeText(info.location, 80) }),
    );
  }
  ...
  if (verificationBits.length > 0) {
    body.appendChild(
      createRow({ icon: "✅", label: "Verification", value: verificationBits.join(" · ") }),
    );
  }
  ...
  if (createdAt) {
    ...
    body.appendChild(
      createRow({
        icon: "🗓️",
        label: "Created",
        value: `${formatShortDate(createdAt)}${ageSuffix}`,
      }),
    );
  }
  ...
  if (typeof meta.verifiedSinceMsec === "number" && meta.verifiedSinceMsec > 0) {
    ...
    if (!Number.isNaN(d.getTime())) {
      body.appendChild(
        createRow({ icon: "⏱️", label: "Verified since", value: formatShortDate(d) }),
      );
    }
  }
  ...
  if (typeof meta.usernameChanges === "number") {
    body.appendChild(
      createRow({ icon: "🔁", label: "Handle changes", value: String(meta.usernameChanges) }),
    );
  }
  ...
  if (errorText) {
    ...
    body.appendChild(
      createRow({ icon: "⏳", label: "Loading", value: "Fetching account details…" }),
    );
  }
