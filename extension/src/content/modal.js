...
export function showModal(
  ...
  } = createCountryBody(blockedCountries, onCountryAction);
  const {
    body: regionBody,
    renderRegions,
    searchInput: regionSearch,
  } = createRegionBody(blockedRegions, onRegionAction);
  const {
    body: tagBody,
    renderTags,
    searchInput: tagSearch,
  } = createTagBody(localBlockedTags, onTagAction);
  ...
  tabBar
    .querySelector('[data-tab="countries"]')
    .addEventListener("click", () => handleTabSwitch("countries"));
  tabBar
    .querySelector('[data-tab="regions"]')
    .addEventListener("click", () => handleTabSwitch("regions"));
  tabBar
    .querySelector('[data-tab="tags"]')
    .addEventListener("click", () => handleTabSwitch("tags"));
...
function createHeader(onClose) {
  ...
  titlePath.setAttribute(
    "d",
    "M12 2L4 5v6.09c0 5.05 3.41 9.76 8 10.91 4.59-1.15 8-5.86 8-10.91V5l-8-3zm6 9.09c0 4-2.55 7.7-6 8.83-3.45-1.13-6-4.82-6-8.83V6.31l6-2.12 6 2.12v4.78zm-9-1.04l-1.41 1.41L10.5 14.5l6-6-1.41-1.41-4.59 4.58z",
  );
...
function createCountryBody(blockedCountries, onAction) {
  const body = createElement("div", {
    className: "x-blocker-body x-blocker-tab-panel",
  ...
  const info = createElement("div", {
    ...
    textContent:
      "Select countries to block. Tweets from users in these countries will be hidden from your feed.",
  });
  ...
  const renderCountries = (filter = currentFilter) => {
    ...
    if (cachedCountryFilter === filterLower && cachedFilteredCountries) {
      ...
      filteredCountries = COUNTRY_LIST.filter((country) => country.includes(filterLower));
...
function createRegionBody(blockedRegions, onAction) {
  ...
  const info = createElement("div", {
    className: "x-blocker-info",
    textContent:
      'Block entire regions. Some users show regional locations like "South Asia" or "Europe" instead of specific countries.',
  });
  ...
  const renderRegions = (filter = currentFilter) => {
    ...
    if (cachedRegionFilter === filterLower && cachedFilteredRegions) {
      ...
      filteredRegions = REGION_LIST.filter(
        (region) =>
...
function createTagBody(blockedTags, onAction) {
  ...
  const info = createElement("div", {
    className: "x-blocker-info",
    textContent:
      "Block users based on emojis, flags, or tags in their display names. Click to toggle blocking.",
  });
