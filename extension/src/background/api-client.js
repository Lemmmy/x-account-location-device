  ...
  async process() {
    ...
    try {
      ...
      if (error instanceof APIError && error.code === API_ERROR_CODES.RATE_LIMITED) {
        // Pessimistic rate limit handling: double the retry-after time to avoid larger buckets and sliding windows
        const baseRetryAfter = error.retryAfter || Date.now() + 60000;
        const waitTime = baseRetryAfter - Date.now();
        this.rateLimitReset = Date.now() + waitTime * 2;
      }
      item.reject(error);
    } finally {
      this.activeRequests--;
  ...
  async dedupe(key, requestFn) {
    ...
    if (this.pending.size >= this.maxSize) {
      ...
      if (firstKey) {
        ...
        console.warn(
          `⚠️ RequestDeduplicator: Evicted oldest entry (${firstKey}), size was ${this.pending.size + 1}`,
        );
      }
    }
...
export class XAPIClient {
  ...
  async executeRequest(screenName, csrfToken = null, retryCount = 0) {
    ...
    if (!headers) {
      throw new APIError("No API headers available", API_ERROR_CODES.NO_HEADERS);
    }
    ...
    try {
      ...
      if (retryCount < API_CONFIG.MAX_RETRIES) {
        ...
        console.warn(
          `⚠️ API request failed for @${screenName}, retrying in ${delay}ms (attempt ${retryCount + 1}/${API_CONFIG.MAX_RETRIES})`,
        );
        await sleep(delay);
      ...
      throw new APIError(error.message || "Network error", API_ERROR_CODES.NETWORK_ERROR);
    }
  }
  ...
  async handleErrorResponse(response) {
    if (response.status === 429) {
      ...
      const baseRetryAfter = reset ? parseInt(reset) * 1000 : Date.now() + 60000;
      // Pessimistic rate limit handling: double the retry-after time internally
      const pessimisticRetryAfter = Date.now() + (baseRetryAfter - Date.now()) * 2;
      const waitMinutes = Math.ceil((pessimisticRetryAfter - Date.now()) / 60000);

      console.warn(
        `⚠️ Rate limited. Retry in ${waitMinutes} minute(s) (pessimistic: 2x Twitter's limit)`,
      );
    ...
    if (response.status === 401 || response.status === 403) {
      ...
      throw new APIError("Authentication failed", API_ERROR_CODES.UNAUTHORIZED, response.status);
    }
    ...
    throw new APIError(`API error: ${response.status}`, API_ERROR_CODES.UNKNOWN, response.status);
  }
  ...
  parseResponse(data) {
    try {
      ...
      const label =
        ...
        user?.identity_profile_labels_highlighted_label?.label ||
        null;
      ...
      throw new APIError("Failed to parse response", API_ERROR_CODES.PARSE_ERROR);
    }
  }
