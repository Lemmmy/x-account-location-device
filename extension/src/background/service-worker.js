...
async function handleSetSettings(newSettings) {
  ...
  // Reinitialize cloud cache if cloud cache settings changed
  if (
    newSettings.cloudCacheEnabled !== undefined ||
    newSettings.cloudCacheApiUrl !== undefined ||
    newSettings.cloudCacheAuthUsername !== undefined ||
    newSettings.cloudCacheAuthPassword !== undefined
  ) {
    await cloudCache.init();
  }
...
async function handleImportData({
  ...
  blockedCountries: importBlockedCountries,
  blockedRegions: importBlockedRegions,
  blockedTags: importBlockedTags,
  cache: importCache,
}) {
  const results = {
    settings: false,
...
function startNotFoundCacheCleanup() {
  ...
  notFoundCleanupInterval = setInterval(() => {
    ...
    if (cleanedCount > 0) {
      console.debug(
        `🧹 Cleaned ${cleanedCount} expired entries from notFoundCache, size: ${notFoundCache.size}`,
      );
    }
