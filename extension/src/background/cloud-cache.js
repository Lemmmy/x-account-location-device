...
class CloudCacheClient {
  constructor() {
    this.enabled = false;
    this.apiUrl = CLOUD_CACHE_CONFIG.DEFAULT_API_URL;
    console.log("CloudCacheClient constructor, API URL: ", this.apiUrl);
   ...
   */
  async init() {
    // Load settings and persisted stats from storage
    ...
    this.enabled = settings.cloudCacheEnabled === true;

    // Load custom API URL or use default
    this.apiUrl = settings.cloudCacheApiUrl || CLOUD_CACHE_CONFIG.DEFAULT_API_URL;
    console.log(
      "CloudCacheClient init, API URL: ",
      this.apiUrl,
      "configured:",
      settings.cloudCacheApiUrl,
      "default:",
      CLOUD_CACHE_CONFIG.DEFAULT_API_URL,
    );
    this.authUsername = settings.cloudCacheAuthUsername || null;
    this.authPassword = settings.cloudCacheAuthPassword || null;
  ...
  recordFailure() {
    ...
    console.warn(
      `☁️ Cloud cache backing off for ${this.getBackoffDelay()}ms (${
        this.consecutiveFailures
      } failures)`,
    );
  }
  ...
  async fetchLookup(usernames) {
    ...
    for (const batch of batches) {
      try {
        ...
        const response = await fetch(`${this.apiUrl}/lookup?users=${batch.join(",")}`, {
          method: "GET",
  ...
  async bulkSync(cacheEntries) {
    if (!this.enabled || !this.isConfigured()) {
      return {
        ...
        skipped: 0,
        errors: 0,
        message: "Cloud cache not enabled",
      };
    }
  ...
  async _fetchServerStatsFromNetwork(timeoutMs) {
    try {
      ...
      return {
        ...
        localEntries: data.localEntries || 0,
        localLookups: data.localLookups || 0,
        cloudflareTotal: data.cloudflareTotal || 0,
        cloudflareDelegated: data.cloudflareDelegated || 0,
