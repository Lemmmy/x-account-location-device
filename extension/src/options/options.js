...
const elements = {
  ...
  cloudAuthUsername: document.getElementById("cloud-auth-username"),
  ...
  cloudLocalEntries: document.getElementById("cloud-local-entries"),
  cloudLocalLookups: document.getElementById("cloud-local-lookups"),
  cloudCfTotal: document.getElementById("cloud-cf-total"),
  cloudCfDelegated: document.getElementById("cloud-cf-delegated"),
  ...
  try {
    ...
    if (response?.success && response.data) {
      ...
      if (elements.optCloudCache) {
        elements.optCloudCache.checked = currentSettings.cloudCacheEnabled === true;
      }

      // Cloud cache config
      if (elements.cloudConfig) {
        elements.cloudConfig.style.display = currentSettings.cloudCacheEnabled ? "block" : "none";
      }
      if (elements.cloudApiUrl) {
        elements.cloudApiUrl.value = currentSettings.cloudCacheApiUrl || "";
      }
      if (elements.cloudAuthUsername) {
        elements.cloudAuthUsername.value = currentSettings.cloudCacheAuthUsername || "";
      }
      if (elements.cloudAuthPassword) {
        elements.cloudAuthPassword.value = currentSettings.cloudCacheAuthPassword || "";
      }
    }
  } catch (error) {
...
function renderCountryGrid(filter = "") {
  ...
  for (const country of filteredCountries) {
    ...
    if (typeof flag === "string" && flag.startsWith("<img")) {
      ...
      const srcMatch = flag.match(
        /src="(https:\/\/abs-0\.twimg\.com\/emoji\/v2\/svg\/[^"]+\.svg)"/,
      );
...
function renderRegionGrid(filter = "") {
  ...
  const filterLower = filter.toLowerCase();
  const filteredRegions = REGION_LIST.filter(
    (region) =>
...
async function fetchCloudServerStats() {
  try {
    ...
    console.log("fetchCloudServerStats response:", response);
    ...
    if (response?.success && response.serverStats) {
      ...
      // Local server stats
      if (elements.cloudLocalEntries) {
        elements.cloudLocalEntries.textContent = Number(stats.localEntries || 0).toLocaleString();
      }
      if (elements.cloudLocalLookups) {
        elements.cloudLocalLookups.textContent = Number(stats.localLookups || 0).toLocaleString();
      }

      // Cloudflare fallback stats
      if (elements.cloudCfTotal) {
        elements.cloudCfTotal.textContent = Number(stats.cloudflareTotal || 0).toLocaleString();
      }
      if (elements.cloudCfDelegated) {
        elements.cloudCfDelegated.textContent = Number(
          stats.cloudflareDelegated || 0,
        ).toLocaleString();
      }
    }
  } catch (error) {
...
function updateCloudCacheUI(enabled, configured, stats) {
  ...
  if (elements.cloudConfig) {
    elements.cloudConfig.style.display = enabled ? "block" : "none";
  }
  ...
  if (stats) {
    ...
    if (elements.cloudContributions) {
      elements.cloudContributions.textContent = stats.contributions || 0;
    }
  }
}
...
function renderStatistics(stats) {
  ...
  const vpnPercentage =
    stats.totalUsers > 0 ? Math.round((stats.vpnCount / stats.totalUsers) * 100) : 0;
  ...
  // Countries item
  const countriesItem = createStatsOverviewItem(
    String(Object.keys(stats.countryCounts).length),
    "Countries",
  );
  statsOverview.appendChild(countriesItem);

  // VPN item
  const vpnItem = createStatsOverviewItem(
    String(stats.vpnCount),
    `🔒 VPN/Proxy (${vpnPercentage}%)`,
  );
...
function setupEventListeners() {
  ...
  if (elements.optCloudCache) {
    elements.optCloudCache.addEventListener("change", async (e) => {
      try {
        await saveSettings({
          cloudCacheEnabled: e.target.checked,
        });

        // Show/hide config section
        if (elements.cloudConfig) {
          elements.cloudConfig.style.display = e.target.checked ? "block" : "none";
        }
  ...
  if (elements.btnSaveCloudConfig) {
    elements.btnSaveCloudConfig.addEventListener("click", async () => {
      try {
        ...
        const authUsername = elements.cloudAuthUsername.value.trim();
        const authPassword = elements.cloudAuthPassword.value;

        await saveSettings({
          cloudCacheApiUrl: apiUrl || "",
          cloudCacheAuthUsername: authUsername || "",
          cloudCacheAuthPassword: authPassword || "",
        });

        // Refresh stats with new config
        await loadCloudCacheStatus();
      } catch (error) {
        console.error("Failed to save cloud config:", error);
        showSaveStatus("Failed to save configuration", true);
      }
    });
  }
  ...
  elements.btnClearCache.addEventListener("click", async () => {
    ...
    if (cacheCount !== "0" && cacheCount !== "-") {
      const confirmed = confirm(
        `Are you sure you want to clear ${cacheCount} cached users?\n\nThis will require re-fetching data for all users.`,
      );
      if (!confirmed) return;
    }
...
async function handleImportFile(file) {
  ...
  try {
    ...
    if (response?.success) {
      ...
      if (response.importedBlockedCountries) {
        results.push(`${response.importedBlockedCountries} blocked countries`);
      }
      if (response.importedBlockedRegions) {
        results.push(`${response.importedBlockedRegions} blocked regions`);
      }
      if (response.importedBlockedTags) {
        results.push(`${response.importedBlockedTags} blocked tags`);
      }
