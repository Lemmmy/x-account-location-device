...
export function findInsertionPoint(container, screenName) {
  ...
  const isProfileHeader =
    ...
    (container.querySelector('[data-testid="userFollowIndicator"]') !== null ||
      (container.getAttribute("data-testid") === "UserName" &&
        container.className.includes("r-14gqq1x")));
  ...
  const handleLink = links.find(
    (l) => l.textContent.trim().toLowerCase() === `@${screenName.toLowerCase()}`,
  );
...
export function extractTagsFromText(text) {
  ...
  const emojiRegex =
    /(?:\p{Emoji_Presentation}|\p{Emoji}\uFE0F|\p{Emoji_Modifier_Base}\p{Emoji_Modifier}?|\p{Emoji_Component})+(?:\u200D(?:\p{Emoji_Presentation}|\p{Emoji}\uFE0F|\p{Emoji_Modifier_Base}\p{Emoji_Modifier}?|\p{Emoji_Component})+)*/gu;
